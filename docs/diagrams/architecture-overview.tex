...
\node[box=ifgreen, minimum width=5.5cm, right=1cm of cli] (mcp)
    {\textbf{MCP Server}\\\textcolor{ifgreen!70!black}{\scriptsize FastMCP -- 32 tools, 16 resources, 4 prompts}};
...
\node[font=\sffamily\scriptsize, align=center, text=syncpurple!80!black] at ([yshift=0.2cm]gitsync.center) {
    Push / Pull\\Section Merge\\Conf{}lict Resolution
};
...
\node[box=domteal, minimum width=3.6cm, minimum height=3.0cm,
...
\node[font=\sffamily\scriptsize, align=left, text=domteal!80!black] at (domain.center) {
    EDA Parsers\\{\scriptsize (6 formats)}\\[2pt]Plugin Registry\\{\scriptsize (\texttt{ctx.importers} entry points)}\\[2pt]GitHub Source\\{\scriptsize (issue bridge)}
};
