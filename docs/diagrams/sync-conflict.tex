...
\begin{tikzpicture}[
    ...
    arr/.style={->, thick, line width=0.8pt, color=#1!70},
    numlabel/.style={
...
% PUSH COLUMN (left)
% ============================
\node[step=ifgreen, below=0.8cm of pushhead] (p1) {Scope filter\\[-1pt]\tiny exclude private/ephemeral};
\node[numlabel=ifgreen, left=0.2cm of p1.west] {1};

\node[step=ifgreen, below=0.7cm of p1] (p2) {Stage files\\[-1pt]\tiny git add scoped entries};
...
% Conflict decision
\node[decision=syncpurple, below=0.8cm of q2] (qconf) {Conflict?};
...
\node[font=\sffamily\tiny, text=domteal!70!black, align=left, below=0.3cm of sagent.south, anchor=north]
    (agentflow) {
        1. persist state\\
        2. \texttt{conflict\_detail()}\\
        3. \texttt{resolve\_conflict()}\\
        4. \texttt{merge\_finalize()}
    };
