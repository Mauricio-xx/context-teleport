...
% === Left: Import sources ===
\node[font=\sffamily\small\bfseries, text=adaptorange!80!black, above left=2.1cm and 4.5cm of store.center]
    (importlabel) {Import Sources};

\node[filebox, below=0.15cm of importlabel] (im1) {\texttt{MEMORY.md} / \texttt{CLAUDE.md}};
\node[filebox, below=0.2cm of im1] (im2) {\texttt{AGENTS.md}, JSON sessions, \texttt{agents/}};
...
\node[font=\sffamily\small\bfseries, text=ifgreen!80!black, above right=2.1cm and 4.5cm of store.center]
...
\node[font=\sffamily\tiny\itshape, text=fsgray, below=0.1cm of im5.south, align=center]
    (skillsnote) {+ \texttt{<tool>/skills/*/SKILL.md}\\for all adapters};

% Plugin registry note
\node[modbox, minimum width=8cm, below=0.4cm of mcpconf] (plugin)
    {\textbf{Plugin Registry:} \texttt{ctx.importers} entry-point group (built-in + third-party)};

\draw[dotted, thick, color=fsgray!50] (mcpconf.south) -- (plugin.north);
